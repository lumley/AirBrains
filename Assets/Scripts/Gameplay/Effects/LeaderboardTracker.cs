...
public class LeaderboardTracker : MonoBehaviour
{
    ...
    public TMP_Text FundsOrRoundText;
    public Color FundsOrRoundTextDefaultColor = Color.white;
    public Color FundsOrRoundTextRoundColor = Color.red;

    private Dictionary<ScoreTracker, LeaderboardEntry> leaderboardEntries =
        new Dictionary<ScoreTracker, LeaderboardEntry>();
    ...
    {
        List<ScoreTracker> sortedTrackers = new List<ScoreTracker>(leaderboardEntries.Keys)
            .OrderByDescending(score => score.Score).ToList();
